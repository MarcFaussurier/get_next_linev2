...
static void* 		ft_buffer_append(char *input, char *outputs[2], size_t *its[2], int (*issep)(char c))
{
	...
	while (input[x])
	{
		outputs[0][its[0][0]] = input[x++];
		if (issep)
			outputs[1][its[1][0]++] = outputs[0][its[0][0]];
		if (issep && issep(outputs[0][its[0][0]]))
		{
			its[0][0] = 0;
			issep = 0;
		}
...
static int 			ft_token_found(t_gnt *ctx, int fd, char *outputs[2], int (*issep)(char c))
{
	size_t			y;
	size_t			z;
